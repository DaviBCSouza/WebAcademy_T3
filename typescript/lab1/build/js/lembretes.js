...
function editarLembrete(userId, index, lembrete) {
...
function removerLembrete(userId, index) {
    let lembretesUser = lembretes[userId];
    if (!lembretesUser) {
        alert("Usuário não encontrado ou sem lembretes");
        return;
...
let formAdicionarLembretes = document.getElementById("formLembretes");
formAdicionarLembretes === null || formAdicionarLembretes === void 0 ? void 0 : formAdicionarLembretes.addEventListener("submit", formAdicionarLembrete);
// Função para realizar a adição dos lembretes do formulário
function formAdicionarLembrete(event) {
    event.preventDefault();
    let formAdicionarLembrete = event.target;
    let tituloEditado = formAdicionarLembrete.elements.namedItem("titulo");
    let dataLimiteEditado = formAdicionarLembrete.elements.namedItem("dataLimite");
    let descricaoEditado = formAdicionarLembrete.elements.namedItem("descricao");
...
function mostrarFormEditar(userId, index) {
    let usuarioAtual = JSON.parse(localStorage.getItem("currentUser") || "{}");
    let lembretesUser = lembretes[userId];
    if (!usuarioAtual ||
    ...
    let formEditarLembretes = document.getElementById("formLembretes");
    let botaoSubmit = document.getElementById("botaoEnviar");
    let tituloInserido = formEditarLembretes.elements.namedItem("titulo");
    let dataLimiteInserido = formEditarLembretes.elements.namedItem("dataLimite");
    let descricaoInserido = formEditarLembretes.elements.namedItem("descricao");
    // Ajustando a hora para o fuso horário local antes de preencher o campo
    let dataLimiteLocal = new Date(lembrete.dataLimite.getTime() -
...
function renderizarListaLembretes() {
    ...
    if (listaLembretes) {
        ...
        if (lembretes.hasOwnProperty(usuarioAtual.usuario)) {
          ...
          <strong>Data de Inserção:</strong> ${formatarDataHora(lembrete.dataInserido)}<br>
          <strong>Data Limite:</strong> ${formatarDataHora(lembrete.dataLimite)}<br>
          <strong>Descrição:</strong> ${lembrete.descricao}<br>
          <button data-user="${usuarioAtual.usuario}" data-index="${index}" class="botaoEditar">Editar</button>
          <button data-user="${usuarioAtual.usuario}" data-index="${index}" class="botaoRemover">Excluir</button>
        `;
